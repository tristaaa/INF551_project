...
def searchkeyword():
    ...
    if (artdata==None and gendata==None and yrdata==None) \
        ...
        for song in songlist:
            if artdata!=None and artdata!='[]':
                arts = song.get('artist').split(',')
                for a in arts:
                    if a.strip() in artdata:
                        result_list.append(song)
                        break
            if gendata!=None and gendata!='[]':
                if song.get('genre') in gendata:
                    result_list.append(song)
            if yrdata!=None and yrdata!='[]':
                if str(song.get('year')) in yrdata:
                    result_list.append(song)
    ...
    total = int(math.ceil(count/10.0)) # total page number
